...
实验室报告中的每个测量通道（δ18O、δ2H、溶质浓度）按三种观测类型参与反演：

- `quantitative`（定量值）：按平方残差 `((预测-观测)/尺度)^2` 参与目标函数；
- `censored`（左删失，即报告“低于检出限”）：检出限不是 0，也不是精确值。该通道按单侧残差 `max(0,(预测-检出限)/尺度)^2` 参与——预测不高于检出限时无惩罚，超出时按超出量惩罚；
- `missing`（真正缺失，如样品量不足未测）：不参与计算。

创建样本时通过 `observations` 字段按通道声明类型，例如 `"observations": {"solute_mg_l": {"type": "censored"}}`；未声明的通道按“有值=定量、空值=缺失”推断。左删失通道必须在数值栏填写报告给出的检出限（如 `<0.1` 填 `0.1`），缺失通道不得携带数值，定量通道必须携带数值，否则返回 422。样本级 `detection_limit` 保留为实验室元数据。

历史样本的原始报告值不会被改写：迁移仅新增 `observation_types` 列（默认 `'{}'`），求解时按上述推断规则确定类型。

可求解性规则：至少 1 项定量观测（删失观测只是不等式约束，无法单独锚定比例），且定量与左删失观测合计至少 2 项。信息不足时，入队与求解都会以 `unsolvable:<原因>` 明确拒绝（`no_quantitative_measurements` 或 `insufficient_measurements`），任务状态记为 `unsolvable`，不会输出看似精确的比例。

每次反演结果的 `observations` 字段逐项列明各通道的观测类型、报告值、参与方式（`squared-residual` / `one-sided-inequality` / `excluded`）、预测值、残差与目标函数贡献，可据此复核每项数据如何参与计算。
